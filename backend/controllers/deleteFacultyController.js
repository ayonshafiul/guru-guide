    ...
    if (error) {
      ...
      for (var i = 0; i < results.length; i++) {
        if (results[i].approve == 0) {
          if (
            results[i].upVoteSum /
              (results[i].upVoteSum + results[i].downVoteSum) >
            0.7
          ) {
...
function removeDuplicate() {
  let sql =
    "DELETE f1 FROM faculty f1 INNER JOIN faculty f2 WHERE f1.upVoteSum < f2.upVoteSum AND f1.facultyInitials = f2.facultyInitials AND f1.departmentID = f2.departmentID";
  db.query(sql, function (error, results) {
    if (error) {
      console.log(error);
    } else {
